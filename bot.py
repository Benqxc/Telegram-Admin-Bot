...
async def autorole_add(ctx, *, role_name: str):
    ...
    if not ctx.author.guild_permissions.administrator:
импортават ctx.отправлять('У вас нет прав администратора.', удалить_после=5)
 возвращаться
...
асинхронный деф autorole_remove(ктх, *, имя_роли: str):
 ...
 если нет ктх.автор.гильдия_разрешения.администратор:
...
если __имя__ == '__основной__':
 ...
 печать('Ошибка: токен не найден. Укажите DISCORD_TOKEN в переменных окружных элементах .env')
 еще:
 бот.бегать(ТОКЕН)
